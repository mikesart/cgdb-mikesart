 ...
 * these routines start_color must be called, usually right after initscr. Colors
 * are always used in pairs (referred to as color-pairs). A color-pair consists of
 * a foreground color (for characters) and a background color (for the blank field
 * on which the characters are displayed). A programmer initializes a color-pair
 * with the routine init_pair. After it has been initialized, COLOR_PAIR(n), a
 * macro defined in <curses.h>, can be used as a new video attribute.
 */
